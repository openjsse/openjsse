...
public class RSAPSSSignature extends SignatureSpi {
    ...
    private RSAKey isValid(RSAKey rsaKey) throws InvalidKeyException {
        try {
            ...
            if (!isCompatible(RSAKeyUtil.getParams(rsaKey), this.sigParams)) {
    ...
    private PSSParameterSpec validateSigParams(AlgorithmParameterSpec p)
        ...
        if (key != null) {
            //JDK8
            if (!isCompatible(RSAKeyUtil.getParams(key), params)) {
                throw new InvalidAlgorithmParameterException
        ...
        //JDK8
        if (params.getTrailerField() != PSSParameterSpec.DEFAULT.getTrailerField()) {
            throw new InvalidAlgorithmParameterException
